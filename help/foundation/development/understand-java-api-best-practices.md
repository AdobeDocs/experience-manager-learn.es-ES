...
description: AEM está construido sobre una rica pila de software de código abierto que expone muchos Java&trade; API para usar durante el desarrollo. Este artículo explora las API principales y cuándo y por qué deben usarse.
version: 6.4, 6.5
feature: APIs
topic: Development
role: Developer
...
# Prácticas recomendadas de la API de Java™
...
AEM se basa en cuatro conjuntos de API Java™ principales.
...
* [**API de AEM JavaDocs**](https://developer.adobe.com/experience-manager/reference-materials/6-5/javadoc/index.html)
...
La variable `com.adobe.cq` paquete admite casos de uso de productos mientras que `com.adobe.granite` admite casos de uso en plataformas de varios productos, como flujos de trabajo o tareas (que se utilizan en todos los productos: AEM Assets, Sitios, etc.).

La variable `com.day.cq` contiene API &quot;originales&quot;. Estas API tratan de abstracciones y funcionalidades básicas que existían antes o alrededor de la adquisición de Adobe de [!DNL Day CQ]. Estas API son compatibles y deben evitarse, a menos que `com.adobe.cq` o `com.adobe.granite` paquetes NO proporciona una alternativa (más reciente).
...
La API preferida es [AEM Query Builder](https://experienceleague.adobe.com/docs/experience-manager-65/developing/platform/query-builder/querybuilder-api.html), que es la abstracción de nivel más alto y proporciona una API sólida para construir, ejecutar y recuperar resultados para consultas, y proporciona lo siguiente:

* Construcción de consultas simple y parametrizada (parámetros de consulta modelados como mapa)
* Nativo [API de Java™ y API de HTTP](https://experienceleague.adobe.com/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions.html?lang=es)
...
* [AEM predicados](https://experienceleague.adobe.com/docs/experience-manager-65/developing/platform/query-builder/querybuilder-predicate-reference.html) compatibilidad con requisitos de consulta comunes

* API extensible, lo que permite el desarrollo de [predicados de consultas](https://experienceleague.adobe.com/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions.html)
...
>[!CAUTION]
>
>AEM API de QueryBuilder filtra un objeto ResourceResolver. Para mitigar esta fuga, siga esta [ejemplo de código](https://github.com/Adobe-Consulting-Services/acs-aem-samples/blob/master/core/src/main/java/com/adobe/acs/samples/search/querybuilder/impl/SampleQueryBuilder.java#L164).
...
La variable [API de JCR (repositorio de contenido de Java™) 2.0](https://developer.adobe.com/experience-manager/reference-materials/spec/javax.jcr/javadocs/jcr-2.0/index.html) forma parte de una especificación para implementaciones JCR (en el caso de AEM, [Apache Jackrabbit Oak](https://jackrabbit.apache.org/oak/docs/)). Toda la implementación de JCR debe cumplir e implementar estas API y, por lo tanto, es la API de nivel más bajo para interactuar con AEM contenido.
...
Siempre prefiera las API de JCR en lugar de las API de Apache Jackrabbit Oak. Las API de JCR son para ***interacción*** con un repositorio JCR, mientras que las API de Oak son para ***implementación*** un repositorio JCR.
...
>[!CAUTION]
>
>El uso generalizado de las interfaces de nodo y sesión de las API de JCR en una aplicación AEM es el olor del código. Asegúrese [!DNL Sling] Las API deben usarse en su lugar.
...
* Creación de estructuras de nodos profundos
   ...
   * Aunque las API de Sling admiten la creación de recursos, las API de JCR tienen métodos de conveniencia en [JcrUtils](https://jackrabbit.apache.org/api/2.12/org/apache/jackrabbit/commons/JcrUtils.html) y [JcrUtil](https://developer.adobe.com/experience-manager/reference-materials/6-5/javadoc/com/day/cq/commons/jcr/JcrUtil.html) que aceleran la creación de estructuras profundas.

## API de OSGi

* [**OSGi R6 JavaDocs**](https://docs.osgi.org/javadoc/r6/cmpn/index.html?overview-summary.html)
* **[OSGi Declarative Services 1.2 Anotaciones de componentes JavaDocs](https://docs.osgi.org/javadoc/r6/cmpn/org/osgi/service/component/annotations/package-summary.html)**
* **[OSGi Declarative Services 1.2 Anotaciones de tipo de metadatos JavaDocs](https://docs.osgi.org/javadoc/r6/cmpn/org/osgi/service/metatype/annotations/package-summary.html)**
* [**OSGi Framework JavaDocs**](https://docs.osgi.org/javadoc/r6/core/org/osgi/framework/package-summary.html)

Hay poca superposición entre las API de OSGi y las API de nivel superior (AEM, [!DNL Sling], y JCR), y la necesidad de utilizar las API de OSGi es escasa y requiere un alto nivel de experiencia en desarrollo AEM.
...
* Anotaciones OSGi para declarar servicios y componentes OSGi.
   ...
   * Preferir [OSGi Declarative Services (DS) 1.2 Anotaciones](https://docs.osgi.org/javadoc/r6/cmpn/org/osgi/service/component/annotations/package-summary.html) over [Anotaciones Félix SCR](https://felix.apache.org/documentation/subprojects/apache-felix-maven-scr-plugin/scr-annotations.html) para declarar servicios y componentes OSGi

* API de OSGi para código de forma dinámica [cancelación/registro de servicios/componentes OSGi](https://docs.osgi.org/javadoc/r6/core/org/osgi/framework/package-summary.html).
...
Cuando se trata de abstracciones OSGi de bajo nivel, como definir o leer en propiedades de componentes OSGi, las abstracciones más recientes proporcionadas por `org.osgi` son preferibles a las abstracciones de Sling de nivel superior. Las abstracciones de Sling de la competencia no se han marcado como `@Deprecated` y sugieran `org.osgi` alternativa.
...
* Preferir [ `com.day.cq.dam.api`](https://developer.adobe.com/experience-manager/reference-materials/6-5/javadoc/com/day/cq/dam/api/package-summary.html) over [ `com.adobe.granite.asset.api`](https://developer.adobe.com/experience-manager/reference-materials/6-5/javadoc/com/adobe/granite/asset/api/package-summary.html).
...
La variable `DamUtil.resolveToAsset(..)` resuelve cualquier recurso en el `dam:Asset` para acceder al objeto Asset , suba por el árbol según sea necesario.
...
La API de recursos proporciona métodos prácticos para leer propiedades desde el `[dam:Asset]/jcr:content/metadata` nodo . No se trata de un ValueMap; no se admite el segundo parámetro (valor predeterminado y conversión de tipo automático).
...
Cuando las propiedades se almacenan en ubicaciones (propiedades o recursos relativos) en las que las API de AEM (página, recurso) no pueden acceder directamente, la variable [!DNL Sling] Se pueden usar los recursos y ValueMaps para obtener los datos.
...
Tenga en cuenta la llamada a `.adaptTo(ModifiableValueMap.class)` requiere permisos de escritura para el recurso; de lo contrario, devuelve nulo.
