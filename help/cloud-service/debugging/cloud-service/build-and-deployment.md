...
Los scripts de repoinit definen el contenido de línea de base, los usuarios, las ACL, etc. AEM En as a Cloud Service AEM, los scripts repoinit se aplican durante la compilación de la imagen, sin embargo, en el inicio rápido local del SDK, se aplican cuando se activa la configuración de fábrica de repoinit de OSGi. AEM Debido a esto, los scripts de Repoinit pueden fallar silenciosamente (con registro) en el inicio rápido local del SDK de la aplicación, pero hacen que el paso Generar imagen falle, lo que detiene la implementación.
...
### La versión de los componentes principales de la aplicación es buena que la versión implementada
...
+ __Síntomas:__
El paso Generar imagen falla con un informe de ERROR que 
`com.adobe.cq.wcm.core.components...` paquetes con intervalos de versiones específicos no se han podido importar mediante la variable `core` proyecto.
...
El paso Implementar en es responsable de tomar el artefacto de código generado en Generar imagen, inicia nuevos servicios de AEM Author y Publish mediante él y, una vez finalizado correctamente, elimina cualquier servicio de AEM Author y Publish antiguo. Los paquetes de contenido mutable y los índices también se instalan y actualizan en este paso.
...
+ __Causa:__ AEM Una canalización de Cloud Manager contiene una versión más antigua de la que se implementa en el entorno de destino. AEM Esto puede ocurrir cuando se reutiliza una canalización y se dirige a un nuevo entorno que ejecuta una versión posterior de la misma Esto se puede identificar comprobando si la versión del entorno en la que se realiza el proceso es más buena AEM que la versión del entorno en la que se realiza el proceso de producción que la versión del proceso que se está ejecutando es más AEM que la versión de la canalización en la que se está.
...
+ __Resolución:__ Revise la implementación para el código que se ejecuta al principio del ciclo de vida del paquete OSGi y revise la `aemerror` registra los servicios de AEM Author y Publish en el momento del error (registre la hora en GMT) como lo muestra Cloud Manager, y busca mensajes de registro que indiquen cualquier proceso de registro personalizado en ejecución.
...
+ __Resolución:__ Revise la `aemerror` registra los servicios de AEM Author y Publish alrededor del tiempo (hora de registro en GMT) del error, como lo muestra Cloud Manager.
...
+ __Causa:__ AEM El usuario del servicio de replicación de datos utilizado para implementar paquetes de contenido en el servicio de publicación de AEM no puede escribir en `/var` en AEM Publish. Esto provoca que la implementación del paquete de contenido en el servicio de publicación de AEM falle.
+ __Resolución:__ Las siguientes formas de resolver estos problemas se enumeran en orden de preferencia:
   ...
   2. Si la variable `/var` Si los recursos son necesarios, defina las estructuras de nodos mediante [repoinit](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/deploying/overview.html#repoinit). Los scripts Repoinit se pueden dirigir a AEM Author, AEM Publish o ambos, a través de los modos de ejecución OSGi.
   3. Si la variable `/var` AEM los recursos solo son necesarios en el autor de la y no se pueden modelar razonablemente con [repoinit](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/deploying/overview.html#repoinit), muévalos a un paquete de contenido discreto que solo esté instalado en AEM Author por [incrustación](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/developing/aem-project-content-package-structure.html?lang=es#embeddeds) que en el `all` paquete en una carpeta de modo de ejecución de Autor de AEM (`<target>/apps/example-packages/content/install.author</target>`).
