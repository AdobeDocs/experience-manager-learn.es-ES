...
La experiencia modal se define mediante la extensión de la aplicación App Builder React definida en la carpeta `web-src`. Como con cualquier aplicación React, la experiencia completa se organiza mediante [Rutas React](https://reactrouter.com/en/main/components/routes) que representan [componentes React](https://reactjs.org/docs/components-and-props.html).

Se requiere al menos una ruta para generar la vista modal inicial. Esta ruta inicial se invoca en la función `onClick(..)` del [registro de extensión](#extension-registration), como se muestra a continuación.
...
Para abrir un modal, se realiza una llamada a `guestConnection.host.modal.showUrl(..)` desde la función `onClick(..)` de la extensión. `showUrl(..)` ha pasado un objeto JavaScript con clave/valores:

+ `title` proporciona el nombre del título del modal mostrado al usuario
+ `url` es la dirección URL que invoca la [ruta React](#modal-routes) responsable de la vista inicial del modal.

Es imperativo que `url` pasado a `guestConnection.host.modal.showUrl(..)` se resuelva en la ruta de la extensión; de lo contrario, no aparecerá nada en el modal.
