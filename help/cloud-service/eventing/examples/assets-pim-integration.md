...
>[!IMPORTANT]
>
>AEM Este tutorial utiliza API de API de basadas en OpenAPI. Están disponibles como parte de un programa de acceso anticipado. Si está interesado en acceder a ellos, le recomendamos que envíe un correo electrónico a [aem-apis@adobe.com](mailto:aem-apis@adobe.com) con una descripción de su caso de uso.

AEM AEM Obtenga información sobre cómo recibir un evento de y actuar en él para actualizar el estado del contenido en mediante el uso de la API de autor de Assets basada en OpenAPI.

La forma de procesar el evento recibido depende de los requisitos empresariales. AEM Por ejemplo, los datos de evento se pueden utilizar para actualizar el sistema de terceros, o para actualizar el sistema de terceros, o para actualizar el sistema de terceros o para actualizar los datos de ambos.

En este ejemplo se muestra cómo se puede integrar un sistema de terceros, como un sistema de administración de la información del producto (PIM), con AEM as a Cloud Service Assets. Al recibir un evento de AEM Assets AEM, se procesa para recuperar metadatos adicionales del sistema PIM y actualizar los metadatos de recursos en la. Los metadatos de recursos actualizados pueden incluir información adicional como el SKU, el nombre del proveedor u otros detalles del producto.
...
1. AEM [Habilitar acceso a las API de](https://experienceleague.adobe.com/en/docs/experience-manager-learn/cloud-service/aem-apis/invoke-openapi-based-aem-apis#enable-aem-apis-access)
...
AEM Para obtener más información sobre los pasos 1-2, consulte la guía [Invocar las API basadas en OpenAPI](https://experienceleague.adobe.com/es/docs/experience-manager-learn/cloud-service/aem-apis/invoke-openapi-based-aem-apis) y, para ver los pasos 3-4, consulte el ejemplo de [Eventos de acción y de de Adobe I/O Runtime AEM](./runtime-action.md#). Para ver los pasos 5-9, consulte las secciones siguientes.
...
- A continuación, seleccione el **Perfil de producto** correcto que está asociado al entorno de AEM Assets desde el cual se produce el evento y tenga acceso suficiente para actualizar los recursos allí. Finalmente, haga clic en el botón **Guardar API configurada**.
  ...
  AEM En mi caso, el perfil de producto _Administradores de - Autor - Programa XXX - Entorno AAAA_ está seleccionado, tiene habilitado el servicio **Usuarios de API de AEM Assets**.
...
- AEM En el proyecto de, busque o cree el archivo `config.yaml` en la carpeta `config`.

  ![Localizar configuración YAML](../assets/examples/assets-pim-integration/locate-config-yaml.png)

- Agregue la siguiente configuración al archivo `config.yaml`.
...
- Implemente los cambios anteriores mediante la canalización de configuración en Cloud Manager. Tenga en cuenta que el archivo `config.yaml` también se puede instalar en un RDE mediante herramientas de la línea de comandos.

  ![Implementar config.yaml](../assets/examples/assets-pim-integration/config-pipeline.png)
...
- AEM El archivo de `src/dx-excshell-1/actions/generic/aemCommunicator.js` actualiza los metadatos de recursos en la mediante la [API de autor de Assets](https://developer.adobe.com/experience-cloud/experience-manager-apis/api/experimental/assets/author/).
...
De forma predeterminada, el proyecto de WKND Sites no tiene el esquema de metadatos de recursos para mostrar los metadatos específicos de PIM como SKU, nombre del proveedor, etc. AEM Vamos a crear y aplicar el esquema de metadatos del recurso a una carpeta de recursos en la instancia de.
...
1. Haga clic en el icono **+** para agregar una nueva pestaña **PIM** y agregar componentes de **Texto de una sola línea**.
   ...
   | --- | --- | --- |
   | SKU | AEM Rellenado automático mediante la integración de ventilación de | `wknd-skuid` |
   | Nombre de Proveedor | AEM Rellenado automático mediante la integración de ventilación de | `wknd-suppliername` |
...
- AEM AEM El código de recuperación de metadatos del recurso se ejecuta fuera de la, lo que evita la carga en el servicio de creación de recursos, por lo que se trata de una arquitectura controlada por eventos que se escala de forma independiente.
...
- AEM Los eventos de activos a través de la ventilación de la permiten a las empresas automatizar y optimizar los procesos críticos, fomentando la eficiencia y la coherencia en todo el ecosistema de contenido.
