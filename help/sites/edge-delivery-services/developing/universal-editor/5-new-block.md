...
- **Contenido de texto**:
   ...
   - **Texto independiente**: contenido descriptivo que proporciona contexto o detalles, incluidos términos y condiciones opcionales.
   - **Botón llamadas a acción (CTA)**: un vínculo diseñado para solicitar la interacción del usuario y guiarlo para que participe en mayor medida.
...
Tenga en cuenta que el bloque `teaser` es similar al bloque `hero` repetitivo; por lo tanto, el bloque `teaser` solo sirve como ejemplo sencillo para ilustrar conceptos de desarrollo.
...
2. **Obtener cambios remotos**: la obtención de las actualizaciones más recientes desde GitHub garantiza que el código más actual esté disponible antes de iniciar el desarrollo.
   - Ejemplo: después de combinar los cambios de la rama `wknd-styles` en la `main`, obtenga las actualizaciones más recientes.
...
Una vez creada la rama `teaser`, podrá comenzar a desarrollar el bloque de teaser.
...
- **Filtro**: configura reglas de filtrado para restringir los contenedores a los que se puede añadir el bloque mediante el Editor universal. La mayoría de los bloques no son contenedores, sino que sus ID se añaden a los filtros de otros bloques de contenedores.
...
1. La experiencia de creación define los campos disponibles para editar.
   ...
   ![Campos del Editor universal](./assets/5-new-block/fields-in-universal-editor.png)

2. Cómo se representan los valores del campo en Edge Delivery Services HTML.

A los modelos se les asigna un `id` que corresponde a la [definición del bloque](#block-definition) e incluyen una matriz `fields` para especificar los campos editables.
...
![Bloque de teaser](./assets/5-new-block/block-design.png)

El bloque de teaser incluye los siguientes elementos editables:

1. **Imagen**: representa el contenido visual del teaser.
2. **Contenido de texto**: incluye el título, el texto independiente y el botón de call-to-action, y se encuentra en un rectángulo blanco.
   - El **título** y **texto independiente** se pueden crear con el mismo editor de texto enriquecido.
   - **CTA** se puede crear a través de un campo `text` para la **etiqueta** y un campo `aem-content` para el **vínculo**.
...
Para comprender las consecuencias de no usar la contracción de campos y la agrupación de elementos, vea la ficha **La manera incorrecta** que se muestra arriba.

>[!TAB La manera incorrecta]
...
El HTML de Edge Delivery Services para el bloque procesa el valor de cada campo en un `div` independiente, lo que complica la comprensión del contenido, la aplicación de estilo y los ajustes de estructura de HTML para lograr el diseño deseado.
...
| Propiedad JSON | Descripción |
|---------------|-------------|
| `definition.title` | El título del bloque tal como se muestra en los bloques **Añadir** del Editor universal. |
| `definition.id` | Identificador único del bloque, usado para controlar su uso en `filters`. |
| `definition.plugins.xwalk.page.resourceType` | Define el tipo de recurso de Sling para procesar el componente en el Editor universal. Use siempre un tipo de recurso de `core/franklin/components/block/v#/block`. |
...
| `definition.plugins.xwalk.page.template.classes` | Propiedad opcional, cuyo valor se añade al atributo `class` del elemento de bloque HTML. Esto permite usar variantes del mismo bloque. El valor `classes` se puede hacer editable al [añadir un campo de clases](https://experienceleague.adobe.com/es/docs/experience-manager-cloud-service/content/edge-delivery/wysiwyg-authoring/create-block#block-options) al [modelo](#block-model) del bloque. |
...
- El bloque incluye contenido predeterminado para el campo `textContent_text`, que es un área de texto enriquecido para el título y el texto independiente, y `textContent_cta` y `textContent_ctaText` para el vínculo y la etiqueta de CTA. Los nombres de campo de la plantilla que contienen contenido inicial coinciden con los nombres de campo definidos en la [matriz de campos del modelo de contenido](#block-model);

Esta estructura garantiza que el bloque esté configurado en el Editor universal con los campos, el modelo de contenido y el tipo de recurso adecuados para el renderizado.

### Filtros de bloque

La matriz `filters` del bloque define, para [bloques de contenedor](https://experienceleague.adobe.com/es/docs/experience-manager-cloud-service/content/edge-delivery/wysiwyg-authoring/content-modeling#container), qué otros bloques se pueden añadir al contenedor. Los filtros definen una lista de identificadores de bloque (`model.id`) que se pueden añadir al contenedor.
...
El identificador de definición de bloque de teaser de `teaser` se añade a la matriz `components`.

## Limpiar los archivos JSON

Asegúrese de [limpiar con frecuencia](./3-local-development-environment.md#linting) los cambios para asegurarse de que estén limpios y sean coherentes. La limpieza suele ayudar a detectar los problemas de forma temprana y reduce el tiempo de desarrollo general. El comando `npm run lint:js` también limpia los archivos JSON y detecta los errores de sintaxis.
...
Después de configurar los archivos JSON de bloque (por ejemplo, `blocks/teaser/_teaser.json`, `models/_section.json`), se compilan automáticamente en los archivos `component-models.json`, `component-definitions.json` y `component-filters.json` del proyecto. Esta compilación se administra automáticamente mediante un vínculo previo a la confirmación de [Husky](https://typicode.github.io/husky/) incluido en la [plantilla del proyecto XWalk de elemento repetitivo de AEM](https://github.com/adobe-rnd/aem-boilerplate-xwalk).
...
## Implementación del bloque JSON
...
Cuando se abre el Editor universal con el parámetro de consulta `?ref=teaser`, el nuevo bloque `teaser` aparece en la paleta de bloques. Tenga en cuenta que el bloque no tiene estilo; renderiza los campos del bloque como HTML semántico, con estilo solo a través de [CSS global](./4-website-branding.md#global-css).
