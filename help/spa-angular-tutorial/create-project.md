---
title: Proyecto del Editor SPA | Introducción al Editor y el Angular de AEM SPA
description: Aprenda a utilizar un proyecto de Adobe Experience Manager (AEM) Maven como punto de partida para una aplicación de Angular integrada con el Editor de SPA de AEM.
sub-product: sitios
...
Aprenda a utilizar un proyecto de Adobe Experience Manager (AEM) Maven como punto de partida para una aplicación de Angular integrada con el Editor de SPA de AEM.
...
En este capítulo, se implementará un nuevo proyecto de AEM, basado en el [AEM Arquetipo de proyecto](https://github.com/adobe/aem-project-archetype). El proyecto AEM se iniciará con un punto de partida muy sencillo para el SPA de Angular. El proyecto utilizado en este capítulo servirá de base para la aplicación del SPA WKND y se basará en futuros capítulos.
...
3. Se utilizaron las siguientes propiedades al generar el proyecto AEM desde el arquetipo [AEM Proyecto](https://github.com/Adobe-Marketing-Cloud/aem-project-archetype/releases/tag/aem-project-archetype-14):
   ...
   >[!NOTE]
   >
   > Observe la propiedad `frontendModule=angular`. Esto indica al arquetipo del proyecto AEM que inicie el proyecto con un [código de Angular base](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/archetype/uifrontend-angular.html) para utilizarlo con el Editor de SPA de AEM.
...
5. Debe ver tres paquetes para `wknd-spa-angular.all`, `wknd-spa-angular.ui.apps` y `wknd-spa-angular.ui.content`.
...
6. También debería ver varios paquetes para `spa.project.core` y `core.wcm.components`. Son dependencias que el arquetipo incluye automáticamente. Puede encontrar más información sobre [Componentes principales de AEM aquí](https://docs.adobe.com/content/help/es-ES/experience-manager-core-components/using/introduction.html).
...
1. En el **[!UICONTROL Editor de páginas]**, haga clic en el menú **[!UICONTROL Información de página]** > **[!UICONTROL Vista tal como se publicó]**:

   ![Botón vista como publicada](./assets/create-project/view-as-published.png)

   Esto abrirá una nueva ficha con el parámetro de consulta `?wcmmode=disabled` que desactiva efectivamente el editor de AEM: [http://localhost:4502/content/wknd-spa-angular/us/en/home.html?wcmmode=disabled](http://localhost:4502/content/wknd-spa-angular/us/en/home.html?wcmmode=disabled)
...
2. Vista el origen de la página y observe que no se encuentra el contenido de texto **[!DNL Hello World]** o cualquiera de los demás contenidos. En su lugar, debería ver HTML como el siguiente:
   ...
   `clientlib-angular.min.js` es la SPA de Angular que se carga en la página y es responsable de procesar el contenido.
...
4. Abra las herramientas de desarrollador del navegador e inspeccione el tráfico de red de la página durante una actualización. Vista de las solicitudes **XHR**:
   ...
   Debería haber una solicitud a [http://localhost:4502/content/wknd-spa-angular/us/en.model.json](http://localhost:4502/content/wknd-spa-angular/us/en.model.json). Contiene todo el contenido, formateado en JSON, que dirigirá la SPA.

5. En una nueva ficha, abra [http://localhost:4502/content/wknd-spa-angular/us/en.model.json](http://localhost:4502/content/wknd-spa-angular/us/en.model.json)
   ...
   En el siguiente capítulo analizaremos cómo se asigna el contenido JSON de AEM componentes a SPA componentes para formar la base de la experiencia del AEM SPA editor.
