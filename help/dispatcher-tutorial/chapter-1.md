...
AEM En este capítulo se proporciona una breve introducción sobre la historia y la mecánica de Dispatcher y se analiza cómo influye esto en la forma en que un desarrollador de la aplicación diseña sus componentes.

## Por qué los desarrolladores deberían preocuparse por la infraestructura

AEM Dispatcher es una parte esencial de la mayoría de las instalaciones, si no de todas las instalaciones de la. Puede encontrar muchos artículos en línea que analizan cómo configurar Dispatcher, así como sugerencias y trucos.

Sin embargo, estos fragmentos de información siempre comienzan en un nivel muy técnico, suponiendo que ya sepa lo que desea hacer y, por lo tanto, solo proporcione detalles sobre cómo lograr lo que desea. Nunca hemos encontrado ningún artículo conceptual que describa la _¿qué es y por qué?_ cuando se trata de lo que puede y no puede hacer con dispatcher.

### Antipatrón: Dispatcher como un pensamiento tardío
...
Es posible que haya oído el consejo de programación _&quot;Primero haz que funcione, luego hazlo rápido&quot;._. No es del todo erróneo. Sin embargo, sin el contexto adecuado, se tiende a malinterpretar y a no aplicarse correctamente.
...
Dispatcher no mejora mágicamente la velocidad del código. Un desarrollador debe crear estos componentes teniendo en cuenta Dispatcher. Por lo tanto, necesita saber cómo funciona.
...
¿Qué es Dispatcher y por qué se llama &quot;Dispatcher&quot;?
...
En los primeros días de la web, se esperaría unos cientos de visitantes a un sitio. AEM La configuración de un Dispatcher, &quot;despachó&quot; o equilibró la carga de solicitudes a una serie de servidores de publicación y eso generalmente era suficiente, por lo tanto, el nombre &quot;Dispatcher&quot;. Sin embargo, en la actualidad ya no se utiliza con mucha frecuencia.
...
2. Dispatcher comprueba si ya tiene una versión procesada de esa página. Supongamos que es la primera solicitud de esta página y que Dispatcher no puede encontrar una copia en caché local.
...
8. Si se solicita la misma página por segunda vez, se puede servir directamente desde la caché de Dispatcher sin necesidad de volver a procesarla en la instancia de publicación. Esto ahorra tiempo de espera para el usuario y ciclos de CPU en la instancia de publicación.
...
En principio, si todos los archivos se almacenan en caché y, por lo tanto, se almacenan estáticamente en Dispatcher, puede extraer el complemento del sistema de publicación y Dispatcher actuaría como un simple servidor web. Pero esto es solo para ilustrar el principio. La vida real es más complicada. No puede almacenar todo en caché, y la caché nunca está completamente &quot;llena&quot;, ya que el número de recursos puede ser infinito debido a la naturaleza dinámica del proceso de renderización. El modelo de un sistema de archivos estático ayuda a generar una imagen aproximada de las capacidades de Dispatcher. Y ayuda a explicar las limitaciones de Dispatcher.
...
* `path/to/resource` es la ruta en la cual el recurso se almacena en CRX y posteriormente en el sistema de archivos del servidor Apache
...
* `#fragment`AEM Sin embargo, la parte de fragmento de una URL no se pasa a la dirección URL, sino que solo se utiliza en el explorador; ya sea en marcos de JavaScript como &quot;parámetros de enrutamiento&quot; o para saltar a una parte determinada de la página.

En Apache (*consulte el diagrama siguiente*),

* `pagename.selectors.html` se utiliza como nombre de archivo en el sistema de archivos de la caché.

Si la dirección URL tiene un sufijo `path/suffix.ext` luego,

* `pagename.selectors.html` se crea como una carpeta

* `path` una carpeta en el `pagename.selectors.html` carpeta

* `suffix.ext` es un archivo en `path` carpeta. Nota: Si el sufijo no tiene una extensión, el archivo no se almacena en caché.
...
1. Las direcciones URL pueden llegar a ser muy largas. Adición de la parte &quot;ruta&quot; de un `/docroot` en el sistema de archivos local podría superar fácilmente los límites de algunos sistemas de archivos. La ejecución de Dispatcher en NTFS en Windows puede ser un desafío. Sin embargo, está seguro con Linux.
...
Las direcciones URL siempre deben tener una extensión. AEM Aunque puede ofrecer direcciones URL sin extensiones en el servicio de direcciones de correo electrónico de. Estas direcciones URL no se almacenarán en caché en Dispatcher.

**Ejemplos**

`http://domain.com/home.html` es **almacenable en caché**

`http://domain.com/home` es **no almacenable en caché**
...
`http://domain.com/home.html/path/suffix` es **no almacenable en caché**
...
Solicitudes posteriores a `home.html/suffix.html` devuelven resultados válidos, pero como archivo `home.html` &quot;bloquea&quot; la posición en el sistema de archivos,  `home.html` no se puede crear por segunda vez como carpeta y, por lo tanto, `home.html/suffix.html` no se almacena en caché.
...
Si lo hace al revés, primero solicite `home.html/suffix.html` entonces `suffix.html` se almacena en caché en una carpeta `/home.html` al principio. Sin embargo, esta carpeta se elimina y se reemplaza por un archivo `home.html` cuando solicite posteriormente `home.html` como recurso.
...
Por lo tanto, el resultado de lo que se almacena en caché es completamente aleatorio y depende del orden de las solicitudes entrantes. Lo que hace que las cosas sean aún más complicadas es el hecho de que normalmente tiene más de un distribuidor. Además, el rendimiento, la tasa de aciertos de caché y el comportamiento pueden variar de forma diferente de una instancia de Dispatcher a otra. Si desea saber por qué su sitio web no responde, debe asegurarse de que está viendo el Dispatcher correcto con el desafortunado orden de almacenamiento en caché. Si está viendo el Dispatcher que, por suerte, tuvo un patrón de solicitud más favorable, se perderá al intentar encontrar el problema.
...
Ahora el usuario carga y activa una nueva versión de ese archivo. AEM Finalmente, se envía una solicitud de invalidación de la base de datos de la instancia de Dispatcher, de la que se hace un seguimiento
...
La invalidación es así de fácil: una solicitud de GET simple a una dirección URL especial &quot;/invalidate&quot; en Dispatcher. No se requiere un cuerpo HTTP, la &quot;carga útil&quot; es solo el encabezado &quot;invalidate-path&quot;. AEM Tenga en cuenta también que la ruta de invalidación en el encabezado es el recurso que conoce el usuario, y no el archivo o archivos que Dispatcher ha almacenado en caché. AEM solo sabe de recursos. Las extensiones, selectores y sufijos se utilizan durante la ejecución cuando se solicita un recurso. AEM no realiza ninguna contabilidad sobre los selectores que se han utilizado en un recurso, por lo que la ruta del recurso es todo lo que sabe con seguridad al activar un recurso.
...
AEM Solo funciona como un encanto, pero si utiliza un Dispatcher en la instancia de publicación, ocurre algo extraño.
...
Publica la página editada &quot;Canada&quot; y vuelve a visitar la página principal publicada anteriormente para ver sus cambios. Pero - nada cambió allí. Sigue mostrando el teaser antiguo. Comprueba dos veces el &quot;Especial de Invierno&quot;. Esa página nunca se ha solicitado antes y, por lo tanto, no se almacena en caché estáticamente en Dispatcher. Por lo tanto, Publish representa recientemente esta página y esta página contiene ahora el nuevo teaser &quot;Visit Canada&quot;.

![Dispatcher almacena contenido incluido obsoleto en la página de inicio](assets/chapter-1/dispatcher-storing-stale-content.png)

*Dispatcher almacena contenido incluido obsoleto en la página de inicio*

<br> 

¿Qué pasó? Dispatcher almacena una versión estática de una página que contiene todo el contenido y el marcado que se ha dibujado desde otros recursos durante el procesamiento.

Dispatcher, siendo un mero servidor web basado en sistemas de archivos, es rápido pero también relativamente simple. Si un recurso incluido cambia, no se da cuenta de eso. Sigue adhiriéndose al contenido que había cuando se representó la página incluida.

La página &quot;Oferta especial de invierno&quot; aún no se ha representado, por lo que no hay ninguna versión estática en Dispatcher y, por lo tanto, se muestra con el nuevo teaser, ya que se procesa recién a petición.
...
1. En un sitio web promedio, solo se solicita con frecuencia un pequeño subconjunto de las páginas. Por lo tanto, incluso si desecha todo el contenido procesado, solo unas pocas docenas se solicitarán inmediatamente después. La renderización de la larga cola de páginas se puede distribuir a lo largo del tiempo, cuando realmente se solicitan. Por lo tanto, la carga en la renderización de páginas no es tan alta como cabría esperar. Por supuesto, siempre hay excepciones... Más adelante analizaremos algunos trucos sobre cómo gestionar cargas igualmente distribuidas en sitios web más grandes con cachés vacías de Dispatcher.
...
![La navegación principal inevitablemente une el contenido de todas las páginas tirando de sus &quot;NavTitles&quot;](assets/chapter-1/nav-titles.png)

*La navegación principal inevitablemente une el contenido de todas las páginas tirando de sus &quot;NavTitles&quot;*
...
¿Puede preguntar por qué se llama &quot;.stat&quot;? ¿Y no quizás &quot;.invalidada&quot;? Bueno, se pueden imaginar, tener ese archivo en el sistema de archivos ayuda a Dispatcher a determinar qué recursos podrían *estáticamente* se puede servir, como si fuera un servidor web estático. Estos archivos ya no necesitan procesarse de forma dinámica.

La verdadera naturaleza del nombre, sin embargo, es menos metafórica. Se deriva de la llamada del sistema Unix `stat()`, que devuelve la hora de modificación de un archivo (entre otras propiedades).

#### Combinación de validación simple y automática

Pero espera... antes dijimos, que los recursos individuales se borran físicamente. Ahora decimos que un archivo de estado más reciente virtualmente los invalidaría a los ojos de Dispatcher. ¿Por qué entonces la eliminación física, primero?
...
2. Si el agente envía un encabezado HTTP `CQ-Action-Scope: ResourceOnly`, esto significa que esta solicitud de invalidación única no déclencheur una invalidación automática. Esto ( [https://github.com/cqsupport/webinar-dispatchercache/tree/master/src/refetching-flush-agent/refetch-bundle](https://github.com/cqsupport/webinar-dispatchercache/tree/master/src/refetching-flush-agent/refetch-bundle)) un fragmento de código podría ser un buen punto de partida para su propio agente de replicación.

3. `ResourceOnly`, solo evita la invalidación automática. Para realizar la resolución de dependencias y las invalidaciones necesarias, debe almacenar en déclencheur las solicitudes de invalidación usted mismo. Es posible que desee comprobar las reglas de vaciado del paquete ([https://adobe-consulting-services.github.io/acs-aem-commons/features/dispatcher-flush-rules/index.html](https://adobe-consulting-services.github.io/acs-aem-commons/features/dispatcher-flush-rules/index.html)) para inspirarse en cómo eso podría realmente suceder.
...
AEM Los recursos se cargan en el área de DAM de solo y en la de los recursos de la _referenciado_ en el componente de imagen adaptable.

El componente de respi se encarga de procesar el marcado y de entregar los datos de imagen binarios.

AEM La forma en que lo implementamos aquí es un patrón común que hemos visto en muchos proyectos e incluso uno de los componentes principales de la se basa en ese patrón. Por lo tanto, es muy probable que usted como desarrollador pueda adaptar ese patrón. Tiene sus puntos fuertes en términos de encapsulación, pero requiere mucho esfuerzo para estar listo para Dispatcher. Discutiremos varias opciones sobre cómo mitigar el problema más adelante.
...
El componente tiene dos partes: la primera parte procesa el marcado del HTML de la imagen y la segunda parte pone en cola los datos binarios de la imagen a la que se hace referencia. Como este es un sitio web moderno con un diseño interactivo, no estamos procesando un `<img src"…">` etiqueta, pero un conjunto de imágenes en `<picture/>` etiqueta. Para cada dispositivo, cargamos dos imágenes diferentes en DAM y las referenciamos desde nuestro componente de imagen.
...
El componente se coloca en el parsys de la página principal. La estructura resultante en CRX se ilustra a continuación.
...
AEM Cada vez que se solicita una página nueva y sin almacenar en caché, los recursos se recuperan de las direcciones URL de las distintas direcciones URLs de la página de destino. Ningún almacenamiento en caché de Dispatcher ni ningún almacenamiento en caché de explorador puede acelerar el envío.
...
Una marca de tiempo Unix es suficientemente buena para una implementación en el mundo real. Para mejorar la legibilidad, utilizamos un formato más legible en este tutorial: `2018 31.12 23:59 or fp-2018-31-12-23-59`.

La huella digital no debe utilizarse como parámetro de consulta, ya que las direcciones URL con parámetros de consulta no pueden almacenarse en caché. Puede utilizar un selector o el sufijo para la huella digital.

Supongamos que es el archivo `/content/dam/flower.jpg` tiene un `jcr:lastModified` fecha del 31 de diciembre de 2018, 23:59. La URL con la huella digital es `/content/home/jcr:content/par/respi.fp-2018-31-12-23-59.jpg`.

Esta URL permanece estable, siempre y cuando el recurso al que se hace referencia (`flower.jpg`) el archivo no ha cambiado. Por lo tanto, se puede almacenar en caché durante un tiempo indefinido y no es un asesino en caché.
...
En nuestro ejemplo, el componente se procesó y se almacenó en caché a las 23:59. Ahora la imagen ha cambiado, digamos a las 00:00.  El componente _haría_ generar una nueva URL con huellas digitales en su marcado.

Puede que lo pienses _debería_... pero no lo hace. Como solo se ha cambiado el binario de la imagen y no se ha tocado la página incluida, no es necesario volver a procesar el marcado del HTML. Por lo tanto, Dispatcher proporciona la página con la huella digital antigua y, por lo tanto, la versión antigua de la imagen.
...
Ahora, si reactiva la página de inicio (o cualquier otra página de ese sitio) en la que se actualizará el archivo de estado, Dispatcher considerará obsoleto el archivo home.html y lo renderizará con una nueva huella digital en el componente de imagen.
...
El administrador diferido suele &quot;_establece la invalidación automática en jpgs y el nivel de archivo de estado en cero, lo que siempre ayuda con los problemas de almacenamiento en caché de todo tipo_.&quot; Encontrarás ese consejo en foros de tecnología, y ayuda con tu problema de invalidación.

Hasta ahora no hemos discutido el nivel del archivo de estado. Básicamente, la invalidación automática solo funciona para archivos del mismo subárbol. Sin embargo, el problema es que las páginas y los recursos generalmente no residen en el mismo subdirectorio. Las páginas están debajo `/content/mysite` mientras que los activos se encuentran por debajo `/content/dam`.
...
De todos modos: tenemos que decirle al Dispatcher de alguna manera, que invalide las páginas HTML si ha cambiado un &quot;.jpg&quot; o &quot;.png&quot; para permitir volver a procesar con una URL nueva.
...
En realidad, no importa qué ruta le diga al Dispatcher que invalide, siempre y cuando esté en el mismo sitio, en el mismo &quot;subárbol&quot;. Ni siquiera tiene que utilizar una ruta de recursos real. También puede ser &quot;virtual&quot;:
...
Cada solicitud omitirá a Dispatcher, lo que provocará la carga en una instancia de publicación. Y, lo que es peor, cree un archivo correspondiente en Dispatcher.

Así que... en lugar de usar la huella digital como un simple asesino en caché, tendría que comprobar la fecha de la imagen jcr:lastModified y devolver un 404 si no es la fecha esperada. Eso toma un poco de tiempo y ciclos de CPU en el sistema de publicación... que es lo que querías evitar en primer lugar.
...
![Poner la imagen en cola con un servlet enlazado a la imagen, no al componente.](assets/chapter-1/spooling-image.png)

*Poner la imagen en cola con un servlet enlazado a la imagen, no al componente.*
...
![Estructura CRX: componente respi2 que agrega una propiedad de calidad al envío](assets/chapter-1/respi2.png)

*Estructura CRX: componente respi2 que agrega una propiedad de calidad al envío*
...
Lo único que falta es el valor de la calidad. El componente sabe qué propiedad introduce el autor. Se podría pasar fácilmente al servlet de renderización de imágenes como parámetro de consulta cuando se procesa el marcado, como `flower.respi2.jpg?quality=60`:
...
![Paso de propiedades de componente como selectores](assets/chapter-1/passing-component-properties.png)

*Paso de propiedades de componente como selectores*
...
De nuevo, esto evita la caché y crea una carga en el sistema de publicación. Por lo tanto, podría ser una mala idea. Puede mitigar esto filtrando solo un pequeño subconjunto de parámetros. Solo desea permitir. `q-20, q-40, q-60, q-80, q-100`.
...
AEM Si no dispone de un cortafuegos de aplicaciones web, debe filtrar en Dispatcher o en el propio servidor de seguridad de la aplicación de la interfaz de usuario de. AEM Si lo hace en la, por favor asegúrese de que

1. El filtro se implementa de forma muy eficiente, sin tener que acceder demasiado al CRX y perder memoria y tiempo.
...
AEM Aquí le está contando a la computadora del navegador. &quot;No tengo... `q-41`. Pero oye... puedes preguntarme sobre `q-40` &quot;.

Esto añade un bucle de solicitud-respuesta adicional a la conversación, que supone un poco de sobrecarga, pero es más barato que realizar el procesamiento completo en `q-41`. Y puede aprovechar el archivo que ya se almacena en caché en `q-40`. AEM Sin embargo, debe comprender que las respuestas 302 no se almacenan en caché en Dispatcher, sino que hablamos de lógica que se ejecuta en la. Una y otra vez. Así que será mejor que lo hagas delgado y rápido.
...
#### Filtrado de Dispatcher y publicación

El siguiente paso es añadir reglas de filtrado de URL en el núcleo de Apache o en Dispatcher.
...
Vaya... la discusión del spooler se hizo más larga y complicada de lo esperado. Te debemos una excusa. Pero sentimos que es necesario presentarle una multitud de aspectos -buenos y malos- para que pueda desarrollar una cierta intuición sobre lo que funciona bien en Dispatcher-land y lo que no.
...
Ya hemos mencionado brevemente la _archivo de estado_ antes. Está relacionado con la invalidación automática:

Todos los archivos de caché del sistema de archivos de Dispatcher configurados para invalidarse automáticamente se consideran no válidos si la fecha de la última modificación es anterior a la `statfile's` fecha de la última modificación.

>[!NOTE]
>
>La fecha de última modificación de la que hablamos es la del archivo en caché, es decir, la fecha en la que el archivo fue solicitado desde el navegador del cliente y finalmente creado en el sistema de archivos. No es el `jcr:lastModified` fecha del recurso.

La fecha de la última modificación del archivo de estado (`.stat`AEM ) es la fecha en la que se recibió la solicitud de invalidación de la aplicación en Dispatcher.

Si tiene más de una instancia de Dispatcher, esto puede producir efectos extraños. Su explorador puede tener una versión más reciente que Dispatcher (si tiene más de un Dispatcher). O una instancia de Dispatcher podría pensar que la versión del explorador emitida por la otra instancia de Dispatcher está obsoleta y envía una copia nueva de forma innecesaria. Estos efectos no tienen un impacto significativo en el rendimiento o en los requisitos funcionales. Y se nivelarán con el tiempo, cuando el navegador tenga la última versión. Sin embargo, puede resultar un poco confuso cuando optimiza y depura el comportamiento de almacenamiento en caché del explorador. Así que ten cuidado.

#### Configuración de dominios de invalidación con /statfileslevel

Cuando introdujimos la invalidación automática y el archivo de estado dijimos: *todo* los archivos se consideran no válidos cuando se produce cualquier cambio y todos los archivos son interdependientes de todos modos.

Eso no es del todo exacto. Normalmente, todos los archivos que comparten una raíz de navegación principal común son interdependientes. AEM Sin embargo, una instancia de puede alojar varios sitios web: *independiente* sitios web. No compartir una navegación común - de hecho, no compartir nada.

¿No sería un desperdicio en invalidar el Sitio B porque hay un cambio en el Sitio A? Sí, lo es. Y no tiene por qué ser así.

Dispatcher proporciona un medio sencillo para separar los sitios entre sí: La variable `statfiles-level`.
...
![/statfileslevel &quot;0&quot;: The_ _.stat_ _se crea en docroot. El dominio de invalidación abarca toda la instalación, incluidos todos los sitios](assets/chapter-1/statfile-level-0.png)

`/statfileslevel "0":` El `.stat` se crea en docroot. El dominio de invalidación abarca toda la instalación, incluidos todos los sitios.

Sea cual sea el archivo invalidado, la variable `.stat` el archivo en la parte superior del docroot de dispatchers siempre se actualiza. Por lo tanto, cuando invalide `/content/site-b/home`, también todos los archivos en `/content/site-a` también se invalidan, ya que ahora son más antiguas que `.stat` en el docroot. Claramente no es lo que necesita, cuando invalida `site-b`.

En este ejemplo, prefiere establecer la variable `statfileslevel` hasta `1`.

Ahora, si publica y, por lo tanto, invalida `/content/site-b/home` o cualquier otro recurso por debajo `/content/site-b`, el `.stat` el archivo se crea en `/content/site-b/`.

Contenido siguiente `/content/site-a/` no se ve afectada. Este contenido se compararía con un `.stat` archivo en `/content/site-a/`. Hemos creado dos dominios de invalidación independientes.

![Un statfileslevel &quot;1&quot; crea diferentes dominios de invalidación](assets/chapter-1/statfiles-level-1.png)

*Un statfileslevel &quot;1&quot; crea diferentes dominios de invalidación*
...
La primera requeriría una `statfileslevel` de _2_, mientras que este último requiere lo siguiente _3_.
...
Estableciéndolo en _2_ significa que en los sitios más grandes está declarando `/canada/en` y `/canada/fr` dependientes, lo que podría no ser. Por lo tanto, cada invalidación en `/en` también invalidaría `/fr`. Esto llevará a una tasa de visitas a la caché ligeramente disminuida, pero sigue siendo mejor que enviar contenido en caché antiguo.
...
Ahora, ¿cuál es el nivel correcto? Depende del número de dependencias que tenga entre los sitios. Las inclusiones que resuelva para procesar una página se consideran &quot;dependencias fijas&quot;. Demostramos tal... _inclusión_ cuando presentamos el _Teaser_ al principio de esta guía.

_Hipervínculos_ son una forma más suave de dependencias. Es muy probable que tenga hipervínculos dentro de un sitio web... y no es improbable que tenga vínculos entre sus sitios web. Los hipervínculos simples no suelen crear dependencias entre sitios web. Piense en un vínculo externo que estableció desde el sitio a facebook... No tendría que procesar su página si algo cambia en facebook y viceversa, ¿verdad?
...
Puede evitar que se le reduzca la clasificación haciendo que sea transparente, que realmente tenga más de una página con el mismo contenido y que no esté intentando &quot;jugar&quot; con el sistema (consulte [&quot;Informar a Google sobre las versiones localizadas de la página&quot;](https://support.google.com/webmasters/answer/189077?hl=en)) mediante la configuración `<link rel="alternate">` a cada página relacionada en la sección del encabezado de cada página:
...
Este esquema creó no solo una serie de vínculos, sino también algunos problemas. El número de vínculos necesarios para _p_ in _n_ idiomas es _p x (n)<sup>2</sup>-n)_: Cada página vincula a otra página (_n x n_) excepto a sí mismo (_-n_). Este esquema se aplica a cada página. Si tenemos un pequeño sitio en 4 idiomas con 20 páginas, cada uno equivale a _240_ Vínculos.
...
En nuestro proyecto, las nuevas páginas relativas aparecían con frecuencia. Pero no se materializaron como enlaces &quot;alternativos&quot;. Por ejemplo, cuando la variable `de-de/produkte` La página fue publicada en el sitio web alemán, no era inmediatamente visible en los otros sitios.
...
`shiny-brand` se ha llamado a `marque-brillant` en Francia y `blitzmarke` en Alemania:
...
Eso habría significado establecer el `statfiles` nivel a 1: lo que habría resultado en un dominio de invalidación demasiado grande.
...
Decidimos quedarnos con el nivel 3 y pagamos el precio de no tener siempre enlaces &quot;alternativos&quot; actualizados. Para mitigarlo, teníamos un trabajo cron &quot;segador&quot; en ejecución en Dispatcher que limpiaba archivos de más de 1 semana de todos modos. Finalmente, todas las páginas fueron renderizadas de todos modos en algún momento. Sin embargo, ese es un equilibrio que debe decidirse individualmente en cada proyecto.

## Conclusión

Cubrimos algunos principios básicos sobre cómo funciona Dispatcher en general y le dimos algunos ejemplos en los que podría tener que realizar un poco más de esfuerzo de implementación para hacerlo bien y en los que podría querer hacer concesiones.
...
Una configuración más fiable está enviando la solicitud de invalidación desde los sistemas de publicación _después_ han recibido el contenido. El artículo &quot;[Invalidar la caché de Dispatcher desde una instancia de publicación](https://helpx.adobe.com/experience-manager/dispatcher/using/page-invalidate.html#InvalidatingDispatcherCachefromaPublishingInstance)&quot; describe los detalles.

**Referencias**

[helpx.adobe.com - Invalidar la caché de Dispatcher desde una instancia de publicación](https://helpx.adobe.com/experience-manager/dispatcher/using/page-invalidate.html#InvalidatingDispatcherCachefromaPublishingInstance)

### Encabezado HTTP y almacenamiento en caché de encabezado

En los viejos tiempos, Dispatcher simplemente almacenaba archivos simples en el sistema de archivos. Si necesitaba que se le enviaran encabezados HTTP al cliente, lo hizo configurando Apache según la poca información que tenía del archivo o la ubicación. AEM Eso fue especialmente molesto cuando implementó una aplicación web en un entorno que se basaba en gran medida en encabezados HTTP. AEM Todo funcionaba bien en la instancia de solo, pero no cuando utilizaba un Dispatcher.

Normalmente empezó a volver a aplicar los encabezados que faltaban a los recursos del servidor Apache con `mod_headers` utilizando información que puede derivar por la ruta de recursos y el sufijo. Pero eso no siempre fue suficiente.

Especialmente molesto fue, que incluso con el Dispatcher el primero _sin almacenar en caché_ La respuesta al explorador provino del sistema de publicación con una gama completa de encabezados, mientras que las respuestas posteriores fueron generadas por Dispatcher con un conjunto limitado de encabezados.

A partir de Dispatcher 4.1.11, Dispatcher puede almacenar los encabezados generados por los sistemas de publicación.

AEM Esto evita la duplicación de lógica de encabezado en Dispatcher y libera todo el poder expresivo de HTTP y.
...
Es posible que desee almacenar en caché todas las páginas e imágenes en general, pero hacer una excepción en algunas circunstancias. Por ejemplo, desea almacenar en caché imágenes PNG, pero no imágenes PNG que muestren un captcha (que se supone que debe cambiar en cada solicitud). AEM Es posible que Dispatcher no reconozca un captcha como captcha... pero ciertamente lo reconoce, ¿no es así? Sí. Puede solicitar a Dispatcher que no almacene en caché esa solicitud única enviando un encabezado correspondiente junto con la respuesta:
...
Cache-Control y Pragma son encabezados HTTP oficiales, que se propagan e interpretan en capas de almacenamiento en caché superiores, como una CDN. El `Dispatcher` El encabezado solo es una sugerencia a Dispatcher para que no almacene en caché. Se puede utilizar para indicar a Dispatcher que no almacene en caché, mientras se permite a las capas de almacenamiento en caché superiores que lo hagan. En realidad, es difícil encontrar un caso en el que eso pueda ser útil. Pero estamos seguros de que hay algunos, en alguna parte.
...
Normalmente, esto se hace de forma estática mediante Apache `mod_expires` AEM o almacenando el Encabezado Cache-Control y Expires que provienen de los archivos de comandos si necesita un control más individual.

Un documento almacenado en caché en el explorador puede tener tres niveles de actualización.

1. _Garantizado fresco_ : El explorador puede utilizar el documento almacenado en caché.

2. _Potencialmente anticuado_ - El explorador debe preguntar primero al servidor si el documento en caché sigue estando actualizado,

3. _Antiguo_ - El navegador debe solicitar al servidor una nueva versión.
...
Para determinar si el documento de la caché del explorador sigue siendo el mismo que se entregaría en una nueva solicitud, el explorador puede utilizar el `Last-Modified` fecha del documento. El explorador pregunta al servidor:

&quot;_Tengo una versión del 10 de junio... ¿necesito una actualización?_&quot; Y el servidor podría responder con

&quot;_304 - Su versión aún está actualizada_&quot; sin volver a transmitir el recurso, o el servidor podría responder con

&quot;_200: esta es una versión más reciente_&quot; en el encabezado HTTP y el contenido real más reciente en el cuerpo HTTP.

Para que funcione la segunda parte, asegúrese de transmitir el `Last-Modified` fecha al explorador, de modo que tenga un punto de referencia para solicitar actualizaciones.

Explicamos anteriormente, que cuando el `Last-Modified` Dispatcher genera una fecha que puede variar entre distintas solicitudes porque el archivo en caché (y su fecha) se genera cuando el explorador solicita el archivo. Una alternativa sería utilizar &quot;etiquetas electrónicas&quot;: se trata de números que identifican el contenido real (por ejemplo, generando un código hash) en lugar de una fecha.

&quot;[Compatibilidad con Etag](https://adobe-consulting-services.github.io/acs-aem-commons/features/etag/index.html)&quot; de la _Paquete ACS Commons_ utiliza este enfoque. Sin embargo, esto tiene un precio: como la etiqueta electrónica debe enviarse como encabezado, pero el cálculo del código hash requiere leer completamente la respuesta, esta debe almacenarse en el búfer completo en la memoria principal antes de poder enviarse. AEM Esto puede tener un impacto negativo en la latencia cuando el sitio web tiene más probabilidades de tener recursos sin caché y, por supuesto, debe estar atento a la memoria consumida por el sistema de la.
...
Utilizamos huellas digitales de URL cuando introdujimos el patrón de cola de impresión. Archivos estáticos procedentes de `/etc/design` (CSS, JS) casi nunca se cambian, lo que los convierte en buenos candidatos para usar como huellas digitales.
...
AEM Las bibliotecas de granite, que conforman la interfaz de usuario de la aplicación, se pueden almacenar en caché durante bastante tiempo. También puede almacenar en caché los archivos estáticos del sitio (fuentes, CSS y JavaScript) en el explorador. Imágenes pares en `/content/dam` normalmente se pueden almacenar en caché durante unos 15 minutos, ya que no se cambian con tanta frecuencia como copiar texto en las páginas. AEM Las imágenes no se editan de forma interactiva en los recursos de la. AEM Se editan y aprueban primero, antes de cargarse en el sitio de trabajo de la interfaz de usuario de. Por lo tanto, puede suponer que no cambian con tanta frecuencia como el texto.
...
AEM Pero no confíes solo en la. Si lo hace, tendría rutas como `/home.html` en el directorio raíz de la caché. Ahora, ¿es esa la &quot;casa&quot; para el sitio web finlandés o alemán o canadiense? Y si hay un archivo `/home.html` en Dispatcher, ¿cómo sabe Dispatcher que esto debe invalidarse cuando se realiza una solicitud de invalidación de? `/content/brand/fi/fi/home` entra.
...
La parte truncada también fue muy fácil.  AEM Se han generado vínculos truncados debido a una configuración acorde en `/etc/map`.

Ahora, cuando una solicitud `/home.html` está accediendo a Dispatcher, lo primero que sucede es aplicar una regla de reescritura que expande internamente la ruta.
...
En el sistema de archivos ahora tenemos sin formato `/content`Rutas basadas en, que se encontrarían también en Author y Publish, lo que ayudó mucho a depurar. Por no hablar de la invalidación correcta - que ya no era un problema.
...
AEM Sí, pero esta es la parte de la, solamente. Recuerde: Dispatcher no almacena en caché `404 – not found` o `500 – internal server error` respuestas.
...
Esto, por supuesto, no es lo que vio el cliente. En Dispatcher, nos registramos `ErrorDocuments` como se muestra:
...
AEM Ahora el sistema de podría notificar al distribuidor que algo estaba mal y el distribuidor podría entregar una versión brillante y hermosa del documento de error.

Hay que señalar dos cosas aquí.

Primero, la variable `error-404.html` siempre es la misma página. Por lo tanto, no hay ningún mensaje individual como &quot;Su búsqueda de&quot;_produkten_&quot;no arrojó un resultado&quot;. Podríamos vivir fácilmente con eso.

AEM AEM En segundo lugar... bueno, si vemos un error interno del servidor - o peor aún, nos encontramos con una interrupción del sistema de la, no hay forma de pedir a los usuarios que procesen una página de error, ¿verdad? La solicitud posterior necesaria según se define en la `ErrorDocument` La directiva también fracasaría. Hemos solucionado ese problema ejecutando un trabajo cron que periódicamente extraía las páginas de error de sus ubicaciones definidas a través de `wget` y almacenarlas en ubicaciones de archivos estáticos definidas en la `ErrorDocuments` Directiva.
...
1. Protect el recurso antes de que la solicitud llegue a la caché, es decir, por una puerta de enlace de inicio de sesión único (SSO) delante de Dispatcher o como módulo en el servidor Apache
...
3. Usar el almacenamiento en caché con permisos confidenciales en Dispatcher

Y, por supuesto, se puede aplicar su propia combinación de los tres enfoques.

**Opción 1**. Su organización podría aplicar una puerta de enlace &quot;SSO&quot; de todas formas. AEM Si el esquema de acceso es muy básico, es posible que no necesite información de para decidir si desea conceder o denegar el acceso a un recurso.

>[!NOTE]
>
>Este patrón requiere un _Gateway_ que _intercepta_ cada solicitud y realiza la _autorización_ : concesión o denegación de solicitudes a Dispatcher. Si su sistema SSO es un _autenticador_, que solo establece la identidad de un usuario que debe implementar en la Opción 3. Si lee términos como &quot;SAML&quot; u &quot;OAauth&quot; en el manual de su sistema SSO, ese es un indicador sólido de que debe implementar la Opción 3.
...
Para mitigar el problema de esta &quot;tormenta de invalidación de caché&quot; como a veces se la llama, puede ser menos riguroso con el `statfile` interpretación.

Puede configurar Dispatcher para que utilice un `grace period` para la invalidación automática. Esto añadiría internamente algo de tiempo adicional a la `statfiles` fecha de modificación.

Digamos, su... `statfile` tiene una hora de modificación de hoy a las 12:00 y `gracePeriod` se establece en 2 minutos. A continuación, todos los archivos invalidados automáticamente se consideran válidos a las 12:01 y a las 12:02. Se vuelven a procesar después de las 12:02.

La configuración de referencia propone una `gracePeriod` de dos minutos por una buena razón. Podrías pensar: &quot;¿Dos minutos? Eso es casi nada. Puedo esperar fácilmente 10 minutos para que el contenido aparezca...&quot;.  Por lo tanto, podría verse tentado a establecer un periodo más largo (por ejemplo, 10 minutos), suponiendo que el contenido se muestre al menos después de estos 10 minutos.

>[!WARNING]
>
>Así no es como `gracePeriod` está funcionando. El periodo de gracia es _no_ el tiempo después del cual se garantiza que un documento se invalidará, pero no se produce ninguna invalidación. Cada invalidación posterior que se incluya en este marco _prolonga_ el lapso de tiempo: esto puede ser indefinidamente largo.

Vamos a ilustrar cómo `gracePeriod` en realidad está trabajando con un ejemplo:
...
12:05 - Otro editor publica su artículo - prolongando el tiempo de gracia por otro gracePeriod a 12:10.

Y así sucesivamente... el contenido nunca se invalida. Cada invalidación *dentro* gracePeriod prolonga efectivamente el tiempo de gracia. El `gracePeriod` está diseñado para capear la tormenta de invalidación... pero debes salir a la lluvia eventualmente... así que, mantén la `gracePeriod` considerablemente corto para evitar esconderse en el refugio para siempre.
...
El `gracePeriod` puede llegar a ser impredeciblemente largo, si su intervalo de replicación regular es más corto que su `gracePeriod`.

La idea alternativa es la siguiente: invalidar solo en intervalos de tiempo fijos. El tiempo intermedio siempre significa ofrecer contenido antiguo. La invalidación se producirá con el tiempo, pero se recopilarán varias invalidaciones para una invalidación &quot;masiva&quot;, de modo que Dispatcher tenga la oportunidad de servir contenido almacenado en caché mientras tanto y darle al sistema de publicación aire para respirar.

La implementación tendría este aspecto:

Utiliza una &quot;Secuencia de comandos de invalidación personalizada&quot; (consulte la referencia) que se ejecutaría después de producirse la invalidación. Este script leería el `statfile's` fecha de la última modificación y redondearla hasta la siguiente parada de intervalo. El comando shell de Unix `touch --time`, vamos a especificar una hora.
...
![Posponer la invalidación a los siguientes 30 segundos completos aumenta la tasa de visitas.](assets/chapter-1/postponing-the-invalidation.png)

*Posponer la invalidación a los siguientes 30 segundos completos aumenta la tasa de visitas.*

<br> 

Las visitas de caché que se producen entre la solicitud de invalidación y la siguiente ranura de ronda de 30 segundos se consideran obsoletas. Se ha producido una actualización en la publicación, pero Dispatcher sigue ofreciendo contenido antiguo.
...
Es muy probable que estas páginas se almacenen en caché en Dispatcher a medida que se solicitan con tanta frecuencia.
...
Con la recuperación automática, puede mitigar eso hasta cierto punto. La mayoría de las páginas invalidadas siguen almacenadas físicamente en Dispatcher después de la invalidación automática. Solo lo son _considerado_ rancio. _Recuperación automática_ significa que, durante el inicio, seguirá usando estas páginas antiguas durante unos segundos _uno solo_ solicitud al sistema de publicación para recuperar el contenido obsoleto:
...
Recuperar significa decir a Dispatcher en cada (!) solicitud de invalidación de que desea recuperar los más populares y cuáles son los más populares.
...
Cuando Dispatcher vea una solicitud de este tipo, almacenará en déclencheur la invalidación automática como de costumbre y pondrá inmediatamente en cola las solicitudes para recuperar contenido nuevo del sistema de publicación.
...
[AEM helpx.adobe.com - Invalidar páginas en la caché de la](https://helpx.adobe.com/experience-manager/dispatcher/using/page-invalidate.html)

### Blindaje del sistema de publicación

Dispatcher proporciona un poco de seguridad adicional al proteger el sistema de publicación de solicitudes que están pensadas solo para fines de mantenimiento. Por ejemplo, usted no quiere exponer usted `/crx/de` o `/system/console` URL para el público.

No es perjudicial tener un firewall de aplicaciones web (WAF) instalado en el sistema. Pero eso añade un número significativo a su presupuesto y no todos los proyectos están en una situación en la que pueden pagar y - no olvidemos - operar y mantener un WAF.
...
En primer lugar, puede reducir el enlace del controlador. `SetHandler` Solo enlaza un controlador a un directorio, podría enlazar el controlador a una URL o a un patrón de URL:
...
AEM Si lo hace, no olvide enlazar siempre el controlador de Dispatcher a la URL de invalidación de Dispatcher; de lo contrario, no podrá enviar solicitudes de invalidación de Dispatcher a Dispatcher desde el punto de vista de la.

Otra alternativa para utilizar Dispatcher como filtro es configurar directivas de filtro en `dispatcher.any`
...
Sin embargo, le proponemos que considere la posibilidad de reducir el espacio de URL lo más pronto posible en la cadena, tanto como sea necesario, y que lo haga de la manera más sencilla posible. Sin embargo, tenga en cuenta que estas técnicas no sustituyen a una WAF en sitios web muy sensibles. Algunas personas llaman a estas técnicas &quot;cortafuegos de hombre pobre&quot; - por una razón.
...
Por suerte, esto ha cambiado en las versiones posteriores de Dispatcher. Ahora también puede utilizar expresiones regulares POSIX y puede acceder a varias partes de una solicitud para definir un filtro. Para alguien que acaba de empezar con Dispatcher eso podría darse por sentado. Pero si estás acostumbrado a tener globos solamente, es una sorpresa y fácilmente se puede pasar por alto. Además, la sintaxis de globs y regex es demasiado similar. Vamos a comparar dos versiones que hacen lo mismo:
...
La versión B utiliza comillas simples `'` para marcar una _patrón de expresión regular_. &quot;Cualquier carácter&quot; se expresa mediante `.*`.

_Patrones de globalización_, por el contrario, utilice comillas dobles `"` y solo puede utilizar marcadores de posición simples como `*`.
...
&quot;Lo reconozco `'/url'` en la configuración... Pero ¿qué es eso? `'/glob'` en el filtro puede preguntar?
...
>[!WARNING]
>
>Configuraciones más sofisticadas dividen las reglas de filtrado en una serie de archivos, que se incluyen en el archivo principal `dispatcher.any` archivo de configuración. Sin embargo, un nuevo archivo no introduce una nueva área de nombres. Por lo tanto, si tiene una regla &quot;001&quot; en un archivo y &quot;001&quot; en otro, obtendrá un error. Aún más razones para encontrar nombres semánticamente fuertes.
...
AEM y, en la mayoría de los casos, Dispatcher funciona de forma predeterminada. Por lo tanto, no encontrará una especificación de protocolo completa de Dispatcher sobre el protocolo de invalidación para crear su propia aplicación en la parte superior. La información es pública, pero un poco dispersa sobre una serie de recursos.
...
`CQ-Action: <action>` - ¿Qué debería pasar? `<action>` es:
...
`CQ-Action-Scope: ResourceOnly` - Opcional: si se establece este encabezado, la variable `.stat` no se ha tocado el archivo.
...
Una buena introducción al almacenamiento en caché de Dispatcher: [https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher.html](https://helpx.adobe.com/es/experience-manager/dispatcher/using/dispatcher.html)

Documentación de Dispatcher con todas las directivas explicadas: [https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html](https://helpx.adobe.com/es/experience-manager/dispatcher/using/dispatcher-configuration.html)

Algunas preguntas frecuentes: [https://helpx.adobe.com/experience-manager/using/dispatcher-faq.html](https://helpx.adobe.com/es/experience-manager/using/dispatcher-faq.html)
...
AEM Invalidar páginas en la caché de la: [https://helpx.adobe.com/experience-manager/dispatcher/using/page-invalidate.html](https://helpx.adobe.com/experience-manager/dispatcher/using/page-invalidate.html)
