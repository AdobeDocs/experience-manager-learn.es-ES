---
title: 'Introducción a AEM Sites: Configuración de proyecto'
...
>[!VIDEO](https://video.tv.adobe.com/v/35883?captions=spa&quality=12&learn=on)
...
**¿Qué es un arquetipo Maven?** - Un [arquetipo Maven](https://maven.apache.org/archetype/index.html) es una plantilla o patrón para generar nuevos proyectos. El arquetipo de proyecto de AEM ayuda a generar un nuevo proyecto con un área de nombres personalizada e incluir una estructura de proyecto que siga las prácticas recomendadas, lo que acelera en gran medida el desarrollo del proyecto.

## Creación del proyecto {#create}

Hay un par de opciones para crear un proyecto de módulo múltiple de Maven para AEM. Este tutorial utiliza el [arquetipo del proyecto Maven AEM **35**](https://github.com/adobe/aem-project-archetype). Cloud Manager también [proporciona un asistente de interfaz de usuario](https://experienceleague.adobe.com/docs/experience-manager-cloud-manager/content/getting-started/project-creation/using-the-wizard.html?lang=es) para iniciar la creación de un proyecto de aplicación de AEM. El proyecto subyacente generado por la interfaz de usuario de Cloud Manager tiene la misma estructura que el uso directo del arquetipo.

>[!NOTE]
>
>Este tutorial utiliza la versión **35** del arquetipo. Se recomienda utilizar siempre la versión **más reciente** del arquetipo para generar un nuevo proyecto.
...
## Implemente y genere el proyecto {#build}

Genere e implemente el código del proyecto en una instancia local de AEM.
...
1. Vaya a la consola Sitios: [http://localhost:4502/sites.html/content](http://localhost:4502/sites.html/content). El sitio WKND es uno de los sitios. Incluye una estructura de sitio con una jerarquía de Masters de EE. UU. y de idiomas. Esta jerarquía del sitio se basa en los valores de `language_country` y `isSingleCountryWebsite` al generar el proyecto mediante el arquetipo.
...
1. El contenido de inicio ya se ha creado y hay varios componentes disponibles para añadirlos a una página. Experimente con estos componentes para hacerse una idea de la funcionalidad. Aprenderá los conceptos básicos de un componente en el capítulo siguiente.
   ...
   ![Contenido inicial para principiantes](assets/project-setup/start-home-page.png)
...
Consulte la [Documentación del arquetipo del proyecto AEM](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/overview.html?lang=es) para obtener más información sobre **todos** los módulos Maven.
...
El entorno AEM as a Cloud Service incluye la última versión de [AEM Core Components](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/introduction.html?lang=es). Por lo tanto, los proyectos generados para AEM as a Cloud Service **no** incluyen una incrustación de AEM Core Components.

Para los proyectos generados por AEM 6.5/6.4, el tipo de archivo incrusta automáticamente [AEM Core Components](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/introduction.html?lang=es) en el proyecto. Se recomienda que AEM 6.5/6.4 incruste los componentes principales de AEM para garantizar que se implementa la versión más reciente con el proyecto. Encontrará más información sobre cómo Core Components [se incluyen en el proyecto aquí](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/using.html?lang=es#core-components).
...
En el módulo `ui.apps` observe que se han creado muchos archivos `.content.xml`. Estos archivos XML asignan los tipos de nodo y las propiedades del contenido instalado en el JCR. Estos archivos son esenciales y **no se pueden** omitir.

El arquetipo del proyecto de AEM genera un archivo de muestra `.gitignore` que puede utilizarse como punto de partida para los archivos que se pueden omitir de forma segura. El archivo se genera en `<src>/aem-guides-wknd/.gitignore`.
...
El módulo **[principal](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/core.html?lang=es)** contiene todo el código Java™ asociado con el proyecto. La compilación del módulo **principal** implementa un paquete OSGi en AEM. Para generar solo este módulo:
...
El módulo maven **[ui.apps](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/uiapps.html?lang=es)** contiene todo el código de renderización necesario para el sitio que se encuentra debajo de `/apps`. Esto incluye CSS/JS que se almacenan en un formato de AEM llamado [clientlibs](https://experienceleague.adobe.com/docs/experience-manager-65/developing/introduction/clientlibs.html?lang=es). Esto también incluye scripts de [HTL](https://experienceleague.adobe.com/docs/experience-manager-htl/content/overview.html?lang=es) para renderizar HTML dinámico. El módulo **ui.apps** es similar a un mapa de la estructura en el JCR pero en un formato que se puede almacenar en un sistema de archivos y comprometerse con el control de código fuente. El módulo **ui.apps** solo contiene código.
...
1. Vuelva a la línea de comandos y ejecute el siguiente comando (en la carpeta `ui.apps`):
   ...
   El perfil `autoInstallPackagePublish` está diseñado para implementar el paquete en un entorno de publicación que se ejecute en el puerto **4503**. El error anterior es previsible si no se encuentra una instancia de AEM que se ejecute en http://localhost:4503.
...
El módulo **[ui.content](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/uicontent.html?lang=es)** tiene la misma estructura que el módulo **ui.apps**. La única diferencia es que el módulo **ui.content** contiene lo que se conoce como contenido **mutable**. El contenido **mutable** hace referencia esencialmente a configuraciones que no son de código, como plantillas, directivas o estructuras de carpetas almacenadas en el control de código fuente, **pero** que se podrían modificar directamente en una instancia de AEM. Esto se analiza con más detalle en el capítulo sobre páginas y plantillas.

Los mismos comandos Maven empleados para compilar el módulo **ui.apps** pueden usarse para compilar el módulo **ui.content**. No dude en repetir los pasos anteriores desde la carpeta **ui.content**.

## Resolución de problemas

Si hay algún problema al generar el proyecto mediante el arquetipo del proyecto de AEM, consulte la lista de [problemas conocidos](https://github.com/adobe/aem-project-archetype#known-issues) y la lista de [problemas abiertos](https://github.com/adobe/aem-project-archetype/issues).

## Enhorabuena de nuevo. {#congratulations-bonus}

Enhorabuena por haber completado el material adicional.

### Próximos pasos {#next-steps-bonus}
