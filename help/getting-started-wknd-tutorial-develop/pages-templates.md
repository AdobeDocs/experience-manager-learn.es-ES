...
1. Compruebe la `pages-templates/start` rama.
...
Siempre puede realizar la vista del código terminado en [GitHub](https://github.com/adobe/aem-guides-wknd/tree/pages-templates/solution) o desproteger el código localmente cambiando a la rama `pages-templates/solution`.
...
## Planificación de la interfaz de usuario con Adobe XD {#adobexd}
...
1. Actualice el encabezado del fragmento de experiencias que se encuentra en [http://localhost:4502/editor.html/content/experience-fragments/wknd/us/en/site/header/master.html](http://localhost:4502/editor.html/content/experience-fragments/wknd/us/en/site/header/master.html) para incluir el logotipo WKND Dark.

   ![Logotipo oscuro WKND](assets/pages-templates/wknd-logo-dk.png)

   *Logo de WKND Dark*

1. Actualice el encabezado del fragmento de experiencias que se encuentra en [http://localhost:4502/editor.html/content/experience-fragments/wknd/us/en/site/footer/master.html](http://localhost:4502/editor.html/content/experience-fragments/wknd/us/en/site/footer/master.html) para incluir el logotipo de WKND Light.
...
1. Crear una nueva plantilla con el tipo de plantilla de página **vacía del sitio** WKND con el título Plantilla de página de **artículo**
...
1. Cambie al modo Contenido **** inicial y agregue los siguientes componentes como contenido inicial:
   ...
   * **Contenedor principal**
      ...
      * Título: *&quot;Por nombre de autor&quot;* con un tamaño de H4
      * Texto - vacío
   * **Contenedor lateral**
      * Título: *&quot;Compartir esta historia&quot;* con un tamaño de H5
...
1. Actualice las Propiedades **de página** iniciales para permitir el uso compartido de usuarios tanto para **Facebook** como para **Pinterest**.
...
1. Habilite las plantillas de página de **artículo** en la carpeta [Plantillas de sitio](http://localhost:4502/libs/wcm/core/content/sites/templates.html/conf/wknd/settings/wcm/templates)WKND.

## Crear una página de artículo

Ahora que tenemos una plantilla, creemos una nueva página con esa plantilla.

1. Descargue el siguiente paquete zip, [WKND-PagesTemplates-DAM-Assets.zip](assets/pages-templates/WKND-PagesTemplates-DAM-Assets.zip) , e instálelo mediante el Administrador [de paquetes](http://localhost:4502/crx/packmgr/index.jsp)CRX.

   El paquete anterior instalará varias imágenes y recursos debajo `/content/dam/wknd/en/magazine/la-skateparks` para que se puedan utilizar para rellenar una página de artículos en pasos posteriores.
...
1. Cree una nueva página, debajo de **WKND** > **EE.UU** > **en**, denominada **Revista**. Utilice la plantilla de página **de** contenido.

   Esta página agregará alguna estructura a nuestro sitio y nos permitirá ver el componente de ruta de navegación en acción.

1. A continuación, cree una nueva página, debajo de **WKND** > **EE.UU** > **en** > **Revista**. Utilice la plantilla **Página** del artículo. Utilice un título de guía **Ultimate para los parques de patinaje** La y un nombre para **guía-la-skateparks**.

   ![Página del artículo creada inicialmente](assets/pages-templates/create-article-page-nocontent.png)

1. Rellene la página con contenido para que coincidan con las maquetas inspeccionadas en la planificación de la [interfaz de usuario con la parte de AdobeXD](#adobexd) . El texto del artículo de muestra se puede [descargar aquí](assets/pages-templates/la-skateparks-copy.txt). Debería poder crear algo similar a esto:
...
## Inspect de la estructura de nodos {#node-structure}
...
1. Abra [CRXDE-Lite](http://localhost:4502/crx/de/index.jsp#/content/wknd/us/en/magazine/guide-la-skateparks/jcr%3Acontent) y utilice la navegación de árbol para desplazarse hasta `/content/wknd/us/en/magazine/guide-la-skateparks`.

1. Haga clic en el `jcr:content` nodo debajo de la `la-skateparks` página y vista las propiedades:

   ![Propiedades de contenido de JCR](assets/pages-templates/jcr-content-properties-CRXDELite.png)

   Observe el valor para `cq:template`, que apunta a `/conf/wknd/settings/wcm/templates/article-page`, la plantilla de página de artículo que hemos creado anteriormente.
...
1. Expanda el `jcr:content` nodo debajo `/content/wknd/us/en/magazine/guide-la-skateparks/jcr:content` y vista la jerarquía de nodos:

   ![JCR Content LA Skateparks](assets/pages-templates/page-jcr-structure.png)

   Debería poder asignar cada uno de los nodos de forma flexible a los componentes creados. Compruebe si puede identificar los diferentes Contenedores de diseño utilizados inspeccionando los nodos con el prefijo `responsivegrid`.
...
1. A continuación, inspeccione el componente de página en `/apps/wknd/components/structure/page`. Vista de las propiedades del componente en CRXDE Lite:
   ...
   Observe la `sling:resourceSuperType` propiedad y el valor de `core/wcm/components/page/v2/page`. Esta propiedad permite que el componente de página de WKND herede toda la funcionalidad del componente de página del componente principal. Este es el primer ejemplo de algo llamado Patrón [de componentes](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/guidelines.html#ProxyComponentPattern)proxy. More information can be found [here.](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/guidelines.html).

1. Inspect otro componente dentro de los componentes WKND, el `Breadcrumb` componente ubicado en: `/apps/wknd/components/content/breadcrumb`. Observe que se puede encontrar la misma `sling:resourceSuperType` propiedad, pero esta vez señala a `core/wcm/components/breadcrumb/v2/breadcrumb`. Este es otro ejemplo de uso del patrón de componentes proxy para incluir un componente principal. De hecho, todos los componentes de la base de código WKND son proxies de AEM componentes principales (excepto nuestro famoso componente HelloWorld). Se recomienda probar y reutilizar la mayor parte posible de la funcionalidad de los componentes principales *antes* de escribir código personalizado.

1. A continuación, revise la página de componentes principales en el `/apps/core/wcm/components/page/v2/page` CRXDE Lite:

   ![Página de componentes principales](assets/pages-templates/core-page-component-properties.png)

   Observe que se incluyen muchas más secuencias de comandos debajo de esta página. La página de componentes principales contiene muchas funciones. Esta funcionalidad se divide en varios scripts para facilitar el mantenimiento y la lectura. Puede rastrear la inclusión de las secuencias de comandos HTL abriendo el `page.html` y buscando `data-sly-include`:
...
1. Inspect es el otro componente principal, como la ruta de exploración en `/apps/core/wcm/components/breadcrumb/v2/breadcrumb`. Vista la `breadcrumb.html` secuencia de comandos para comprender cómo se genera finalmente el marcado para el componente de ruta de navegación.
...
Por ahora, trataremos las plantillas como otras partes de código y sincronizaremos la plantilla **de página de** artículo como parte del proyecto. Hasta ahora hemos **insertado** código de nuestro proyecto AEM en una instancia local de AEM. La plantilla **de página de** artículo se creó directamente en una instancia local de AEM, por lo que necesitamos **extraer** o importar la plantilla en nuestro proyecto AEM. El módulo **ui.content** se incluye en el proyecto AEM para este propósito específico.

Los siguientes pasos se llevarán a cabo mediante el IDE de Eclipse, pero podrían estar haciendo uso de cualquier IDE que haya configurado para **extraer** o importar contenido de una instancia local de AEM.
...
1. Expanda el módulo **ui.content** en el explorador de proyectos. Expanda la `src` carpeta (la que tiene el icono de un pequeño gob) y navegue hasta `/conf/wknd/settings/wcm/templates`.
...
1. [!UICONTROL Haga clic] con el botón derecho en el `templates` nodo y seleccione **Importar desde el servidor...**:
   ...
   Confirme el cuadro de diálogo **Importar desde repositorio** y haga clic en **Finalizar**. Ahora debería ver el `article-page-template` debajo de la `templates` carpeta.

1. Repita los pasos para importar contenido pero seleccione el nodo de **políticas** ubicado en `/conf/wknd/settings/wcm/policies`.

   ![Directivas de importación de eclipse](assets/pages-templates/policies-article-page-template.png)

1. Inspect el `filter.xml` archivo ubicado en `src/main/content/META-INF/vault/filter.xml`.
...
En este punto, la página del artículo está claramente sin estilo. Siga el tutorial [Biblioteca del lado del cliente y Flujo de trabajo](client-side-libraries.md) front-end para conocer las prácticas recomendadas para incluir CSS y Javascript para aplicar estilos globales al sitio e integrar una compilación del front-end dedicada.

Vista el código terminado en [GitHub](https://github.com/adobe/aem-guides-wknd) o revise e implemente el código localmente en la llave de Git `pages-templates/solution`.

1. Clona el repositorio de [github.com/adobe/aem-wknd-guides](https://github.com/adobe/aem-guides-wknd) .
1. Compruebe la `pages-templates/solution` rama.
