---
sub-product: sitios
topics: development
version: cloud-service
doc-type: tutorial
activity: develop
audience: developer
user-guide-title: 'Introducción a AEM Sites: Tutorial de WKND'
user-guide-description: Un tutorial de varias partes para desarrolladores que van a usar AEM por primera vez. Implementar un sitio AEM para una marca ficticia de ropa, WKND.
breadcrumb-title: Tutoriales de WKND de AEM Sites
index: y
translation-type: tm+mt
source-git-commit: e03d84f92be11623704602fb448273e461c70b4e
workflow-type: tm+mt
source-wordcount: '56'
ht-degree: 66%

---


# Introducción a AEM Sites: Tutorial de WKND {#getting-started-wknd-tutorial-develop}

+ [Información general del tutorial de WKND](overview.md)
+ [1 - Configuración del proyecto](project-setup.md)
+ [2 - Conceptos básicos de componentes](component-basics.md)
+ [3: Páginas y plantillas](pages-templates.md)
+ [4 - Bibliotecas del lado del cliente](client-side-libraries.md)
+ [5 - Sistema de estilos](style-system.md)
+ [6 - Componente personalizado](custom-component.md)
+ [7 - Prueba de unidad](unit-testing.md)
