...
>[!NOTE]
>
> También es posible seguir el tutorial con un entorno as a Cloud Service AEM. Se incluyen notas adicionales sobre el uso de un entorno de Cloud en todo el tutorial.
...
1. Inicie sesión con el nombre de usuario `admin` y contraseña `admin`.
...
1. En el IDE de su elección, abra el archivo `.env.development` at `aem-guides-wknd-graphql/react-app/.env.development`. Compruebe que la variable `REACT_APP_AUTHORIZATION` línea no tiene comentarios y el archivo tiene el siguiente aspecto:
   ...
   >[!NOTE]
   >
   > En un escenario de producción, la aplicación se conectará a un AEM **Publicación** entorno. Esto se explica con más detalle en la sección [Implementación de producción](production-deployment.md) capítulo.
...
1. Vaya al IDE de GraphiQL en [http://localhost:4502/content/graphiql.html](http://localhost:4502/content/graphiql.html) y comenzar a explorar las API de GraphQL.
   ...
   >[!NOTE]
   >
   > La herramienta GraphiQL y la API de GraphQL son [explorado con más detalle más adelante en el tutorial](./explore-graphql-api.md).
