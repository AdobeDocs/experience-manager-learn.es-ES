---
title: 'Consulte AEM mediante GraphQL desde una aplicación externa: Introducción a AEM Headless - GraphQL'
description: Introducción a Adobe Experience Manager (AEM) y GraphQL. Explore las API de AEM GraphQL como ejemplo de la aplicación WKND GraphQL React. Descubra cómo esta aplicación externa realiza llamadas de GraphQL a AEM para potenciar su experiencia. Obtenga información sobre cómo realizar la gestión de errores básica.
sub-product: activos
topics: headless
...
# Consultar AEM mediante GraphQL desde una aplicación externa

En este capítulo, analizamos cómo se pueden utilizar las API de GraphQL de AEM para impulsar la experiencia en una aplicación externa.

Este tutorial utiliza una aplicación React simple para consultar y mostrar contenido de Aventura expuesto por las API de GraphQL de AEM. El uso de React no es muy importante, y la consumidora aplicación externa podría ser escrita en cualquier marco para cualquier plataforma.
...
* Explore cómo se realizan llamadas desde la aplicación externa a los puntos finales de GraphQL de AEM
...
1. Revise la aplicación en [http://localhost:3000/](http://localhost:3000/). La aplicación React de ejemplo tiene dos partes principales:
   ...
   * La experiencia principal actúa como un índice de WKND Adventures, consultando los __Fragmentos de contenido de aventura__ en AEM mediante GraphQL. En este capítulo, modificaremos esta vista para admitir el filtrado de aventuras por actividad.
...
1. En su IDE, abra el archivo `src/api/useGraphQL.js`.
   ...
   Se trata de un [React Effect Hook](https://reactjs.org/docs/hooks-overview.html#effect-hook) que escucha los cambios en la aplicación `query`, y al cambiar realiza una solicitud HTTP POST al punto final de AEM GraphQL y devuelve la respuesta JSON a la aplicación.

   Cada vez que la aplicación React necesita realizar una consulta de GraphQL, invoca este enlace personalizado `useGraphQL(query)`, pasando GraphQL para enviarlo a AEM.

   Este enlace utiliza el módulo simple `fetch` para realizar la solicitud HTTP POST GraphQL, pero otros módulos como el cliente [Apollo GraphQL](https://www.apollographql.com/docs/react/) pueden usarse de manera similar.
...
1. Abra `src/components/AdventureDetail.js`, el componente React responsable de mostrar la experiencia de detalles de aventura. Esta vista solicita un fragmento de contenido específico, utiliza su ruta JCR como su identificador único y procesa los detalles proporcionados.
   ...
   De forma similar a `Adventures.js`, el enlace `useGraphQL` React Hook personalizado se reutiliza para realizar la consulta de GraphQL con AEM.
...
1. Inspeccione la función `filterQuery(activity)`, que no se utiliza, pero que se ha preparado para formular una consulta de GraphQL que filtra las aventuras por `activity`.
...
1. Guarde los cambios y vuelva a cargar la aplicación React en el navegador web. Los tres nuevos botones aparecen en la parte superior y, al hacer clic en ellos, se vuelve a consultar automáticamente AEM para fragmentos de contenido de aventura con la actividad correspondiente.
...
1. Vuelva a abrir el archivo `src/api/useGraphQL.js`. Inspeccione el siguiente fragmento de código para ver la administración de errores:
...
1. Modifique `allAdventuresQuery` para incluir una propiedad no válida `adventurePetPolicy`:
   ...
   */
   ...
   Sabemos que `adventurePetPolicy` no forma parte del modelo de aventura, por lo que esto debería generar un error.
...
1. Inspeccione la respuesta de AEM utilizando las herramientas para desarrolladores del explorador para ver el objeto JSON `errors`:
