...
9. Inspect utiliza **app.module.ts** en `ui.frontend/src/app/app.module.ts`.
   ...
   El **TextComponent** no se incluye explícitamente, sino que se incluye dinámicamente mediante **AEMResponsiveGridComponent** proporcionado por el SDK de JS AEM Editor. Por lo tanto, debe aparecer en la matriz **app.module.ts**&#39; [entryComponents](https://angular.io/guide/entry-components).
...
2. Cree un nuevo componente Imagen ejecutando el comando CLI `ng generate component` del Angular desde la carpeta `ui.frontend`:
...
3. En el IDE, abra **image.component.ts** en `ui.frontend/src/app/components/image/image.component.ts` y actualice como se indica a continuación:
   ...
   `ImageEditConfig` es la configuración para determinar si se va a procesar el marcador de posición de autor en AEM, en función de si se rellena la  `src` propiedad.
...
5. Abra **image.component.scss** y actualícelo de la siguiente manera:
   ...
   >[!NOTE]
   >
   > La regla `:host-context` es **crítica** para que el marcador de posición del editor de SPA de AEM funcione correctamente. Todos los componentes de SPA que se pretenda crear en el editor de páginas de AEM necesitarán esta regla como mínimo.
...
El componente `ImageComponent` solo es visible en el **servidor de desarrollo de webpack**. A continuación, implemente la SPA actualizada para AEM y actualizar las directivas de plantilla.

1. Detenga el **servidor de desarrollo de webpack** y, desde **root** del proyecto, implemente los cambios en AEM con sus habilidades con Maven:
...
2. En la pantalla Inicio de AEM, vaya a **[!UICONTROL Herramientas]** > **[!UICONTROL Plantillas]** > **[Angular de SPA de WKND](http://localhost:4502/libs/wcm/core/content/sites/templates.html/conf/wknd-spa-angular)**.

   Seleccione y edite la **SPA Página**:
...
4. En **Componentes permitidos** > **Angular de SPA WKND - Contenido** > compruebe el componente **Imagen**:
...
Enhorabuena, ha aprendido a asignar SPA componentes a AEM componentes y ha implementado un nuevo componente `Image`. También tiene la oportunidad de explorar las capacidades de respuesta del **Contenedor de diseño**.

Siempre puede ver el código terminado en [GitHub](https://github.com/adobe/aem-guides-wknd-spa/tree/Angular/map-components-solution) o extraer el código localmente cambiando a la rama `Angular/map-components-solution`.

### Pasos siguientes {#next-steps}

[Navegación y enrutamiento](navigation-routing.md) : Descubra cómo se pueden admitir varias vistas de la SPA asignando páginas AEM con el SDK del Editor de SPA. La navegación dinámica se implementa mediante el enrutador de Angular y se agrega a un componente de encabezado existente.
